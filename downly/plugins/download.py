...
async def download(client: Client, message: Message):
    ...
    async def download_stream(user_message: Message, downloader_instance):
        ...
        # sending video
        await send_video(
            message=message, video=downloaded_file, progress=progress.progress
        )
    ...
    if domain in ["youtube.com", "youtu.be"]:
        downloader = YoutubeDownloader(
            youtube_url=user_url_message, output_dir=output_dir
        )
    ...
    try:
        ...
        logger.error(
            f"Error occurred while processing {user_url_message}\n"
            f"Error message: {e}"
        )
    ...
    # logging output
    logger.info(
        f"handing request for {user_url_message} with output {output} - "
        f"from {title}({id})"
    )
    ...
    if output.get("status") == "stream":
        ...
        downloader = StreamDownloader(
            url=output.get("url"),
            output_path=str(Path.resolve(output_dir / "[STREAM_FILENAME]")),
        )
    ...
    if output.get("status") == "redirect":
        ...
        try:
            ...
            await first_message.edit_text(
                "An error occurred. Please try again later." f"\nError details: `{e}`"
            )
            return

        await first_message.delete()
        logger.info(
            f"finished handling request for {user_url_message} - " f"from {title}({id})"
        )
        return
